...
def pack(VOX_VERSION_NUMBER):
    ...
    print 'Packaging Voxelent GUI. Version:' + str(VOX_VERSION_NUMBER)
    ...
    gui_files = [os.path.join(root,name)
                 for root, dirs, files in os.walk(".."+SEP+"source"+SEP+"gui") 
                 for name in files
                 if name.endswith(".js")]

    for f in gui_files:                         #the separator replacements of this section always apply because we are calling java
    ...
    for lkp in gui:
        ...
        if key in hashmap:
            buffer += hashmap[key]
            print "Adding " + key + " to vox-gui"+VERSION_TAG+".js"
        if key_min in hashmap:
            buffer_min += hashmap[key_min]
            print "Adding " + key_min + " to vox-gui"+VERSION_TAG+"-min.js"
    
    demos_dir = open('..'+SEP+'demos'+SEP+'lib'+SEP+'vox-gui'+VERSION_TAG+'.js','w+');
    demos_dir_min = open('..'+SEP+'demos'+SEP+'lib'+SEP+'vox-gui'+VERSION_TAG+'-min.js','w+');
    lib_dir = open('..'+SEP+'downloads'+SEP+'vox-gui'+VERSION_TAG+'.js','w+');
    lib_dir_min = open('..'+SEP+'downloads'+SEP+'vox-gui'+VERSION_TAG+'-min.js','w+');
